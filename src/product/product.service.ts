  ...
  async create(createProductDto: CreateProductDto): Promise<Product> {
    try {
      ...
      if(!product.expiry_date) {
        throw new Error('Não é possivel cadastrar um produto sem data de validade');
      }

      if(this.checkExpiryDate(product.expiry_date)) {
        throw new Error('Não é possivel cadastrar um produto vencido');
      }
  ...
  async update(id: number, updateProductDto: UpdateProductDto) {
    try {
      if(this.checkExpiryDate(new Date(updateProductDto.expiry_date))) {
        throw new Error('Não é possivel atualizar um produto vencido');
      } 
